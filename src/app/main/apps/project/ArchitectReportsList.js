...
const ArchitectReportsList = props => {
	...
	useEffect(() => {
		fetch('http://localhost:3001/reports/')
			...
			.then(response => {
				...
				return response
					.filter(res => parseInt(res.projectId) === parseInt(props.id))
					.map(res => {
						return { id: res.id, projectId: res.projectId, date: res.date };
					});
			})
	...
	return (
		<div
			...
			style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', margin: '5%' }}
		>
			{project.length === 0 ? (
							...
							<Link key={value} to={link}>
								<ListItem role={undefined} dense button>
									...
									<ListItemText
										...
										secondary={new Date(date).toTimeString()}
									/>
